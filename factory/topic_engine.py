...
def load_topics(paths):
    return [Topic(**x) for x in read_json(paths.topics_json,[]) if isinstance(x,dict)]
...
def find_resumable_job(paths, processor="qwen"):
    ...
    candidates = []
    for n in os.listdir(root):
        if not n.startswith("BH"): continue
        manifest = read_json(paths.manifest(n),{}) or {}
        if manifest.get("claimed_by") != processor: continue
        status = _job_status(paths,n)
        if status not in {"QWEN_READY","COMPLETED"}:
            candidates.append((n,manifest.get("topic_id")))
    candidates.sort()
...
def claim_next_topic(paths, processor="qwen"):
    ...
    for topic in topics:
        ...
        write_json_atomic(paths.manifest(job_id),{
            "job_id":job_id,"topic_id":topic.id,"title":topic.title,
            "created_at":now_iso(),"claimed_by":processor,"status":"QWEN_RESEARCHING"
        })
        write_json_atomic(paths.state(job_id,"qwen"),{
            "status":"CLAIMED","updated_at":now_iso(),"processor":processor
        })
        return topic, job_id
    return None, None
...
def mark_used(paths, topic):
    ...
    topics = load_topics(paths)
    for item in topics:
        if item.id == topic.id: item.used = True
