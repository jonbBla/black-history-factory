...
def _maybe_publish(paths, data):
    try:
        ...
        push_status(type('C',(),{'github_repo':repo,'github_dashboard_path':cfg.get('github_dashboard_path','dashboard/data')})(),data,history,token)
        data['_last_github_push']=time.time()
        write_json_atomic(paths.status_current,data)
    except Exception as e:
        data.setdefault('warnings',[]).append(f'GitHub publish warning: {e}')

def set_processor(paths,name,status,job_id='',stage='',detail='',completed=0,total=0):
    data=read_json(paths.status_current,{}) or {}
    data.setdefault('processors',{})[name]={
        'status':status,'job_id':job_id,'stage':stage,'detail':detail,
...
def append_history(paths, job_id, title, video_path, thumbnail_path, seconds=None):
    ...
    item={'job_id':job_id,'title':title,'video':video_path,'thumbnail':thumbnail_path,'date':now_iso()}
    if seconds is not None: item['seconds']=seconds
    history.insert(0,item); write_json_atomic(paths.status_history,history)
    return history

def mark_idle(paths,name): set_processor(paths,name,'idle')
