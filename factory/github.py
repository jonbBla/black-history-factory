...
dashboard repo's dashboard/data/ folder, via the GitHub Contents API, so the
GitHub Pages dashboard reflects live progress without Colab needing git
installed or SSH keys configured — just a fine-grained PAT with Contents:
read/write on this one repo, read from Colab's Secrets panel as GH_TOKEN.

The dashboard never talks to Colab directly; it only ever reads these two
JSON files from the repo.
"""
...
def _put_file(repo: str, path: str, content: dict, token: str, message: str) -> None:
